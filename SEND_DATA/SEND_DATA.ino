...
uint64_t LONG_TIMEOUT = 3600;
uint64_t SHORT_TIMEOUT = 10;
...
const int power_level = 18;
const int EchoPin1 = 39;
const int TriggerPin1 = 26;
float level = 0.0;

//Load sensor
const int power_weight = 2;
...
void setup()
{
  ...
  //POWER PINS
  pinMode(power_hum, OUTPUT);
  pinMode(power_bme280, OUTPUT);
  pinMode(power_gas, OUTPUT);
  pinMode(power_weight, OUTPUT);
  pinMode(power_level, OUTPUT);
  pinMode(power_temp, OUTPUT);

  //INICIALIZAR
  Serial.println("Inicializando Sensores");
  digitalWrite(power_gas, LOW);
  digitalWrite(power_hum, LOW);
  digitalWrite(power_bme280, HIGH);
  digitalWrite(power_weight, HIGH);
  digitalWrite(power_temp, HIGH);
  digitalWrite(power_level, LOW);
  ...
  checkRed = connectToWiFi(ssid_string.c_str(), password_string.c_str(), true);

  if (!checkRed)
  {
    connectToBT();
    bool ConnVerif = false;
    int Count = 0;
    while (!ConnVerif)
    { 
      DataReceive1 = false;
      DataReceive2 = false;
      BT_data();
      Serial.println("El SSID de conexion es: " + STRssid);
      Serial.println("El PASSWORD de conexion es: " + STRpassword);
      ConnVerif = connectToWiFi(STRssid.c_str(), STRpassword.c_str(), false);
      if (Count > 3)
      {
        goToDeepSleep(SHORT_TIMEOUT);
      }
      Count++;
    }
  ...
  digitalWrite(power_gas, HIGH);
  delay(45000);
  digitalWrite(power_hum, HIGH);
  digitalWrite(power_level, HIGH);
  delay(15000);
  ...
  Serial.println("Voy a dormir 1 hora......");
  goToDeepSleep(LONG_TIMEOUT);
}
...
void goToDeepSleep(uint64_t Time)
{
  //Serial.println("Voy a domir durante " + Time.c_str()  + "segundos!!!");
...
bool connectToWiFi(const char *WIFI_NETWORK, const char *WIFI_PASSWORD, bool verif)
{
...
void connectToBT()
{
  ...
  SerialBT.begin("El bluetooth inicio, esperando a emparejarse con otro dispositivo!");
